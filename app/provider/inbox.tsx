...
export default function ProviderInboxScreen() {
    ...
    const handleNotificationPress = async (item: any) => {
        ...
        if (item.type === 'payment' || item.type === 'withdrawal') router.push('/provider/earnings');
        else if (item.type === 'job_offer') router.push('/provider/requests');
        else if (item.project_id) router.push(`/provider/project/${item.project_id}`);
        else if (item.type === 'verification') router.push('/provider/profile');
    ...
    return (
        <ScreenGradient>
            ...
            <NavigationBar title="Inbox" showBack onMenuPress={() => router.replace('/provider')} dynamicColor={theme.colors.emerald} />
            {loading ? (
