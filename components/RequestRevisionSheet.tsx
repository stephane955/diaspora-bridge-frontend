...
export default function RequestRevisionSheet({
  visible,
  milestoneTitle,
  loading,
  onClose,
      ...
      >
        ...
        >
          ...
          <View style={styles.titleRow}>
            <Ionicons name="refresh-circle" size={24} color={PREMIUM_GOLD} />
            <Text style={styles.title}>{t('requestRevision')}</Text>
          </View>
          <Text style={styles.sub}>
            {milestoneTitle
              ? `${milestoneTitle} — ${t('requestRevisionSub')}`
              : t('requestRevisionSub')}
          </Text>
          <TextInput
            style={styles.input}
            placeholder={t('requestRevisionPlaceholder')}
          ...
          <View style={styles.actions}>
            ...
            >
              {loading ? (
                ...
                <Text style={styles.submitText}>{t('sendRevision')}</Text>
              )}
            </TouchableOpacity>
          </View>
        </BlurView>
...
const styles = StyleSheet.create({
  ...
  submitText: { color: PREMIUM_BG, fontWeight: '800', fontSize: 15 },
});
